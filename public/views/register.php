...
$content = '
    <div style="position: relative; min-height: calc(100vh - 60px); margin-top: 60px; background: linear-gradient(135deg, #f7fafc, #e6f0fa); overflow: hidden;">
        ...
        <div style="max-width: 400px; margin: 0 auto; padding: 2rem; background: rgba(255, 255, 255, 0.95); border-radius: 1rem; box-shadow: 0 8px 16px rgba(0, 0, 0, 0.1); position: relative; z-index: 2; text-align: center;">
            ...
            <h2 style="color: #f59e0b; font-size: 1.75rem; font-weight: 700; margin-bottom: 1.5rem;">Inscription</h2>
            <form method="POST" action="?action=register" style="display: flex; flex-direction: column; gap: 1rem;">
                <input type="hidden" name="referred_by" value="' . htmlspecialchars($_GET['ref'] ?? '') . '">
                <div style="text-align: left;">
                    <label style="display: block; color: #374151; font-weight: 600; margin-bottom: 0.5rem;">Nom</label>
                    <input type="text" name="name" required style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 0.5rem; font-size: 1rem;">
                </div>
                <div style="text-align: left;">
                ...
                <button type="submit" style="background-color: #3b82f6; color: white; padding: 0.75rem; border: none; border-radius: 0.5rem; cursor: pointer; transition: background-color 0.3s;">S\'inscrire</button>
            </form>
            <p style="margin-top: 1rem; color: #4a5568;">Déjà inscrit ? <a href="?action=login" style="color: #3b82f6; text-decoration: none; font-weight: 600;">Connectez-vous ici</a></p>
        </div>
    </div>
';
...
<script>
    ...
    // Mettre à jour le placeholder du numéro de téléphone (inchangé)
